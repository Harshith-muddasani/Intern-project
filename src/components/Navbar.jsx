...
export default function Navbar({ user, onLogout, onProfile, onViewSessions, onAdminPanel, onSendNewsletter = () => {}, onSharingSettings = () => {} }) {
  ...
  return (
    <header className="dark-navbar sticky top-0 shadow-sm z-50">
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 w-full">
          ...
          <div className="flex items-center space-x-4 ml-auto">
              ...
              <div className="relative" ref={userDropdownRef}>
                ...
                {userDropdownOpen && (
                  ...
                  >
                    ...
                    >
                      <span style={{fontSize: '1.1em', color: '#ff5e62'}}>•</span>
                      <span>{t('yourSavedAltars')}</span>
                    </a>
                    <a 
                      href="#" 
                      onClick={e => { e.preventDefault(); setUserDropdownOpen(false); onSharingSettings(); }} 
                      className="block px-4 py-2 text-sm transition-colors duration-200 hover:opacity-80 no-underline flex items-center gap-2"
                      style={{ color: 'var(--theme-text)', textDecoration: 'none' }}
                      onMouseEnter={handleDropdownMouseEnter}
                      onMouseLeave={handleDropdownMouseLeave}
            ...
            {!user && <Button onClick={() => navigate('/login')}>{t('login')}</Button>}
          </div>
        </div>
      </nav>
    </header>
